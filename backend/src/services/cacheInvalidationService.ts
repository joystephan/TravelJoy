...
class CacheInvalidationService {
   ...
   */
  async invalidateWeatherCache(lat: number, lon: number): Promise<number> {
    const patterns = [
    ...
    for (const pattern of patterns) {
      deletedCount += await cacheUtils.delPattern(pattern);
    }

    console.log(
      `Invalidated ${deletedCount} weather cache entries for location (${lat}, ${lon})`
    );
    return deletedCount;
  }
   ...
   */
  async invalidatePlaceCache(query: string): Promise<number> {
    const pattern = `nominatim:search:${query}:*`;
    const deletedCount = await cacheUtils.delPattern(pattern);

    console.log(
      `Invalidated ${deletedCount} place cache entries for query: ${query}`
    );
    return deletedCount;
  }
   ...
   */
  async invalidateGeocodeCache(address: string): Promise<number> {
   ...
   */
  async invalidateReverseGeocodeCache(
    lat: number,
    lon: number
  ): Promise<number> {
    ...
    console.log(
      `Invalidated reverse geocode cache for coordinates (${lat}, ${lon})`
    );
    return deleted ? 1 : 0;
  }
  ...
  async invalidateCountryCache(
    ...
    if (!countryCode && !countryName) {
      ...
      deletedCount = await cacheUtils.delPattern("countries:*");
    }
   ...
   */
  async invalidateAllExternalApiCache(): Promise<{
    weather: number;
    places: number;
    countries: number;
    total: number;
    ...
    const [weather, places, countries] = await Promise.all([
      cacheUtils.delPattern("weather:*"),
      cacheUtils.delPattern("nominatim:*"),
      cacheUtils.delPattern("countries:*"),
    ]);
   ...
   */
  async invalidateCacheByAge(
    pattern: string,
    maxAgeSeconds: number
  ): Promise<number> {
    try {
      const keys = await cacheUtils.delPattern(pattern);
      // Note: This is a simplified implementation
      // For more precise age-based invalidation, store timestamps with cache entries
      console.log(
        `Invalidated ${keys} cache entries matching pattern: ${pattern}`
      );
      return keys;
    } catch (error) {
  ...
  schedulePeriodicCleanup(intervalHours: number = 24): () => void {
    ...
    const intervalId = setInterval(async () => {
      ...
      await cacheUtils.delPattern("weather:current:*");
