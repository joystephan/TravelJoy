...
class IntegrationTester {
    ...
    async testUserRegistration() {
        try {
            ...
            this.addResult("User Registration", false, `Failed: ${error.response?.data?.error || error.message}`);
            return false;
        }
    }
    async testUserLogin() {
    ...
    async testSubscriptionFlow() {
        try {
            ...
            if (!plansResponse.data || plansResponse.data.length === 0) {
                this.addResult("Subscription Flow", false, "No subscription plans available");
                return false;
            }
            this.addResult("Subscription Flow - Get Plans", true, `Found ${plansResponse.data.length} plans`, {
                plans: plansResponse.data.map((p) => p.name),
            });
            // Check subscription status
            const statusResponse = await this.api.get("/api/subscription/status");
            this.addResult("Subscription Flow - Check Status", true, "Status retrieved", {
                status: statusResponse.data,
            });
            return true;
        }
        catch (error) {
            this.addResult("Subscription Flow", false, `Failed: ${error.response?.data?.error || error.message}`);
            return false;
        }
    }
    async testTripCreation() {
        try {
            ...
            this.addResult("Trip Creation", false, `Failed: ${error.response?.data?.error || error.message}`);
            return false;
        }
    }
    async testItineraryGeneration() {
        try {
            if (!this.tripId) {
                this.addResult("Itinerary Generation", false, "No trip ID available");
                return false;
            }
            const response = await this.api.post(`/api/trips/${this.tripId}/generate`);
            if (response.data.itinerary && response.data.itinerary.length > 0) {
                this.addResult("Itinerary Generation", true, `Generated ${response.data.itinerary.length} days`, {
            ...
            this.addResult("Itinerary Generation", false, `Failed: ${error.response?.data?.error || error.message}`);
            return false;
        }
    }
    async testTripRetrieval() {
        try {
            ...
            this.addResult("Trip Retrieval", false, `Failed: ${error.response?.data?.error || error.message}`);
            return false;
        }
    }
    async testChatAssistant() {
        try {
            if (!this.tripId) {
            ...
            this.addResult("Chat Assistant", false, `Failed: ${error.response?.data?.error || error.message}`);
            return false;
        }
    }
    async runAllTests() {
