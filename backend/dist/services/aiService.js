...
class AIService {
    ...
    async generateCompletion(prompt, systemPrompt) {
        ...
        try {
            await redis_1.default.setex(cacheKey, 3600, response);
        }
        catch (error) {
     ...
     */
    async generateOllamaCompletion(prompt, systemPrompt) {
        try {
     ...
     */
    async generateHuggingFaceCompletion(prompt, systemPrompt) {
        if (!this.huggingfaceApiKey) {
            throw new Error("HuggingFace API key not configured");
        }
        try {
            const fullPrompt = systemPrompt ? `${systemPrompt}\n\n${prompt}` : prompt;
            const response = await axios_1.default.post(`https://api-inference.huggingface.co/models/${this.model}`, {
                inputs: fullPrompt,
                parameters: {
                    max_new_tokens: 2000,
                    temperature: 0.7,
