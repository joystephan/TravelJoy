# TravelJoy - App Store Metadata

## App Information

### App Name

TravelJoy - AI Travel Planner

### Subtitle (iOS) / Short Description (Android)

AI-powered travel planning made simple

### Description

**Full Description (4000 characters max)**

Transform your travel planning with TravelJoy, the intelligent mobile app that creates personalized multi-day itineraries in seconds. Powered by advanced AI, TravelJoy takes the stress out of trip planning by generating optimized schedules tailored to your preferences, budget, and travel style.

**Key Features:**

🤖 AI-Powered trip Generation
Let our smart AI create detailed day-by-day travel plans including activities, dining recommendations, and transportation options. Just tell us where you want to go, your budget, and preferences – we'll handle the rest.

🗺️ Interactive Maps & Navigation
Visualize your entire trip on interactive maps with all your activities, restaurants, and hotels marked. Get directions and explore your destination with confidence.

☀️ Real-Time Weather Integration
Plan better with integrated weather forecasts for your destination. Our AI adjusts recommendations based on weather conditions to ensure you have the best experience.

💬 Smart Travel Assistant
Chat with our AI assistant for instant travel advice, restaurant recommendations, and quick trip modifications. Ask questions in natural language and get helpful responses.

💰 Budget-Conscious Planning
Set your budget and let TravelJoy optimize your trip to maximize value. Track estimated costs for activities, meals, and transportation throughout your trip.

✈️ Multi-Day Trip Planning
Create comprehensive travel plans for weekend getaways or extended vacations. Our AI considers opening hours, travel times, and optimal routing for seamless experiences.

🎯 Personalized Recommendations
Customize your experience with preferences for:

- Activity types (cultural, adventure, relaxation, nightlife)
- Food preferences (local cuisine, fine dining, casual)
- Transportation (public transit, walking, driving)
- Schedule pace (relaxed, moderate, packed)

📱 Offline Access
Access your saved trips even without internet connection. Perfect for international travel when data is limited.

🔄 Easy Modifications
Don't like a suggestion? Easily edit or replace any activity with alternatives. Your trip adapts to your changes instantly.

**Perfect For:**

- Solo travelers seeking adventure
- Couples planning romantic getaways
- Families organizing vacations
- Business travelers with free time
- Anyone who wants stress-free travel planning

**Subscription Plans:**

Free Trial: 7 days with 1 trip generation
Basic Plan: $9.99/month - 5 trips per month
Premium Plan: $19.99/month - Unlimited trips + priority AI processing

Start planning your dream vacation today with TravelJoy!

---

### Keywords (iOS - 100 characters max)

travel planner,trip planner,trip,vacation,AI travel,travel guide,trip organizer,travel app

### Keywords (Android - separate by commas)

travel planner, trip planner, trip maker, vacation planner, AI travel assistant, travel guide, trip organizer, travel app, destination guide, travel recommendations, trip builder, holiday planner

### Category

Primary: Travel
Secondary: Navigation (iOS) / Maps & Navigation (Android)

### Age Rating

4+ (iOS) / Everyone (Android)

### Privacy Policy URL

https://traveljoy.com/privacy-policy

### Terms of Service URL

https://traveljoy.com/terms-of-service

### Support URL

https://traveljoy.com/support

### Marketing URL

https://traveljoy.com

## Screenshots Requirements

### iOS Screenshots

**iPhone 6.7" (iPhone 14 Pro Max, 15 Pro Max)**

- Resolution: 1290 x 2796 pixels
- Required: 3-10 screenshots

**iPhone 6.5" (iPhone 11 Pro Max, XS Max)**

- Resolution: 1242 x 2688 pixels
- Required: 3-10 screenshots

**iPad Pro 12.9" (3rd gen)**

- Resolution: 2048 x 2732 pixels
- Optional but recommended

### Android Screenshots

**Phone**

- Minimum: 320px
- Maximum: 3840px
- Required: 2-8 screenshots

**7-inch Tablet**

- Optional: 1024 x 600 pixels minimum

**10-inch Tablet**

- Optional: 1280 x 800 pixels minimum

### Screenshot Suggestions

1. **Welcome/Onboarding Screen**

   - Show the app's value proposition
   - Highlight AI-powered features

2. **Trip Creation Screen**

   - Demonstrate easy input process
   - Show destination search with autocomplete

3. **Generated trip**

   - Display beautiful day-by-day schedule
   - Show activity cards with images

4. **Interactive Map View**

   - Highlight map integration
   - Show activity markers and routes

5. **Chat Assistant**

   - Demonstrate AI chat interface
   - Show helpful travel recommendations

6. **Weather Integration**

   - Display weather-aware planning
   - Show forecast integration

7. **Subscription Plans**

   - Clear pricing display
   - Feature comparison

8. **Trip Details/Editing**
   - Show easy modification interface
   - Demonstrate flexibility

## App Preview Videos (Optional but Recommended)

### iOS App Preview

- Duration: 15-30 seconds
- Resolution: Same as screenshot requirements
- Format: .mov, .m4v, or .mp4

### Android Promo Video

- Duration: 30 seconds - 2 minutes
- Resolution: 1920 x 1080 (16:9) recommended
- Format: MPEG-4 or WebM

### Video Content Suggestions

1. Quick app walkthrough (5 seconds)
2. Creating a trip (10 seconds)
3. Viewing generated trip (8 seconds)
4. Using chat assistant (7 seconds)
5. Call to action (5 seconds)

## App Icon

### iOS

- 1024 x 1024 pixels
- PNG format
- No transparency
- No rounded corners (iOS adds them)

### Android

- Adaptive icon: 512 x 512 pixels
- Foreground and background layers
- PNG format

### Design Guidelines

- Simple, recognizable design
- Works well at small sizes
- Represents travel/journey theme
- Uses brand colors (#4A90E2 blue)

## Promotional Text (iOS - 170 characters)

Plan your perfect trip in seconds with AI. Get personalized itineraries, smart recommendations, and real-time travel assistance. Start your adventure today!

## What's New (Release Notes)

### Version 1.0.0

Welcome to TravelJoy! 🎉

This is our initial release featuring:

✨ AI-powered trip generation
🗺️ Interactive maps with OpenStreetMap
☀️ Real-time weather integration
💬 Smart travel assistant chat
💰 Budget-conscious planning
📱 Offline trip access
🎯 Personalized recommendations

We're excited to help you plan amazing trips!

Have feedback? Contact us at support@traveljoy.com

## Localization

### Initial Launch

- English (US) - Primary

### Future Localization (Planned)

- Spanish (ES, MX)
- French (FR)
- German (DE)
- Italian (IT)
- Portuguese (BR)
- Japanese (JP)
- Chinese Simplified (CN)

## Contact Information

### Developer Name

TravelJoy Inc.

### Support Email

support@traveljoy.com

### Privacy Email

privacy@traveljoy.com

### Phone Number

+1 (555) 123-4567

## Testing Information for Review

### Test Account Credentials

Email: reviewer@traveljoy.com
Password: ReviewTest123!

### Test Payment Information

Use Stripe test card: 4242 4242 4242 4242
Any future expiry date, any CVC

### Demo Instructions

1. Log in with test account
2. Create a new trip to "Paris, France"
3. Set budget to $2000, dates 7 days from now
4. Select preferences and generate trip
5. Explore map view and chat assistant
6. Test subscription flow with test card

### Notes for Reviewers

- AI generation may take 30-60 seconds
- Requires internet connection for initial setup
- Location permission is optional but enhances experience
- Subscription is required for more than 1 trip during trial

## App Store Optimization (ASO) Strategy

### Primary Keywords to Target

- travel planner
- trip planner
- trip maker
- AI travel
- vacation planner

### Competitor Analysis

- TripIt
- Sygic Travel
- Roadtrippers
- Google Trips (discontinued)

### Unique Selling Points

1. AI-powered automation (vs manual planning)
2. Budget optimization
3. Weather-aware recommendations
4. Natural language chat assistant
5. Completely free external APIs (no hidden costs)

## Launch Checklist

- [ ] App icon finalized (1024x1024)
- [ ] Screenshots prepared (all required sizes)
- [ ] App preview video created (optional)
- [ ] Privacy policy published
- [ ] Terms of service published
- [ ] Support website live
- [ ] Test account created
- [ ] App Store Connect account set up
- [ ] Google Play Console account set up
- [ ] Stripe production keys configured
- [ ] Production backend deployed
- [ ] App metadata translated (if applicable)
- [ ] Beta testing completed
- [ ] Legal review completed
- [ ] Marketing materials prepared
